...
contract StakingPool is StakingRewardsPool {
     ...
     *
     */
    function strategyDeposit(uint256 _index, uint256 _amount, bytes calldata _data) external onlyOwner {
    ...
    //@audit can the max deposit be incresed and used to take out a flash loan and explout the protocol.
    /**
     ...
     */
    //@audit can the min be set to high and unusable for the user?
    ...
    function getStrategyDepositRoom() external view returns (uint256) {
        uint256 depositRoom;
        for (uint256 i = 0; i < strategies.length; ++i) {
            uint256 strategyDepositRoom = IStrategy(strategies[i]).canDeposit();
     ...
     *
     */
    //@audit can onlyowner be exploited?
    function removeStrategy(uint256 _index, bytes memory _strategyUpdateData, bytes calldata _strategyWithdrawalData)
        external
        onlyOwner
    {
        require(_index < strategies.length, "Strategy does not exist");
     ...
     *
     */
    //@audit can the onlyOwner be expøoited. correct varibales?ß
     ...
     *
     */
    //@audit varibles correctly updated?
    ...
    //@audit can the brun function be exploited to destory the protocol?
    /**
    ...
    function _updateStrategyRewards(uint256[] memory _strategyIdxs, bytes memory _data) private {
        ...
        if (totalFeeAmounts > 0) {
            ...
            //@audit can this nested for-loop be exploited???
