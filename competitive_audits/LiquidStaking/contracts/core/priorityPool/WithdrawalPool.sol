...
contract WithdrawalPool is UUPSUpgradeable, OwnableUpgradeable {
    ...
    // @audit can somone use this to reenter?
    /**
    ...
    //@audit nested for loop. can this be DOS?ßß
    /**
    ...
    function withdraw(uint256[] calldata _withdrawalIds, uint256[] calldata _batchIds) external {
        ...
        for (uint256 i = 0; i < _withdrawalIds.length; ++i) {
            ...
            if (
                batchId != 0 && withdrawalId > batch.indexOfLastWithdrawal
    ...
    function performUpkeep(bytes calldata _performData) external {
        ...
        if (totalQueued == 0 || canWithdraw == 0 || block.timestamp <= timeOfLastWithdrawal + minTimeBetweenWithdrawals)
        {
            revert NoUpkeepNeeded();
        }
        //@audit    why uint64 and not uint256?
        timeOfLastWithdrawal = uint64(block.timestamp);
    ...
    //@audit does this handle edge cases?
    /**
    ...
    //@audit is the varibales corectly updated?
    /**
