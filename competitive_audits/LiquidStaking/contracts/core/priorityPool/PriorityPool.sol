...
contract PriorityPool is UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    event UpdateDistribution(
        bytes32 merkleRoot, bytes32 ipfsHash, uint256 incrementalAmount, uint256 incrementalSharesAmount
    );
    ...
    function canWithdraw(address _account, uint256 _distributionAmount) external view returns (uint256) {
        uint256 canUnqueue =
            paused() ? 0 : MathUpgradeable.min(getQueuedTokens(_account, _distributionAmount), totalQueued);
        uint256 stLINKCanWithdraw =
            MathUpgradeable.min(stakingPool.balanceOf(_account), stakingPool.canWithdraw() + totalQueued - canUnqueue);
    ...
    function claimLSDTokens(uint256 _amount, uint256 _sharesAmount, bytes32[] calldata _merkleProof) external {
        address account = msg.sender;
        //@audit is this correct use of keccak?
        bytes32 node = keccak256(bytes.concat(keccak256(abi.encode(account, _amount, _sharesAmount))));
     ...
     */
    function depositQueuedTokens(uint256 _queueDepositMin, uint256 _queueDepositMax, bytes[] calldata _data) external {
    ...
    function checkUpkeep(bytes calldata) external view returns (bool, bytes memory) {
        ...
        if (strategyDepositRoom < queueDepositMin || (totalQueued + unusedDeposits) < queueDepositMin) {
            return (false, "");
        }
        ...
        return (
            ...
            abi.encode(
                MathUpgradeable.min(
                    MathUpgradeable.min(strategyDepositRoom, totalQueued + unusedDeposits), queueDepositMax
                )
            )
        );
    }
     ...
     */
    //@audit can account data be manipulated?
     ...
     */
    //@audit can the orecle be manipulated?
...
//@audit one more function for future upgrades?
