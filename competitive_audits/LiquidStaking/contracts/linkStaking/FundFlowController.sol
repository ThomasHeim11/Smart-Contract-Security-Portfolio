...
contract FundFlowController is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function updateVaultGroups() external {
        ...
        if (
            curUnbondedGroup != 0 && timeOfLastUpdateByGroup[curUnbondedGroup] == 0
    ...
    function updateOperatorVaultGroupAccounting(uint256[] calldata _vaultGroups) external {
        ...
        for (uint256 i = 0; i < _vaultGroups.length; ++i) {
            (uint256 depositRoom,) =
                _getTotalDepositRoom(vaults, numVaultGroups, _vaultGroups[i], maxDeposits, depositIndex);
        ...
        IOperatorVCS(address(operatorVCS)).updateVaultGroupAccounting(
            _vaultGroups, totalDepositRoom, totalUnbonded, maxDeposits
        );
    }
        ...
        returns (uint256[] memory, uint256, uint256)
    {
        ...
        (uint256 curGroupTotalDepositRoom, uint256[] memory curGroupVaultsToUnbond) =
            _getTotalDepositRoom(vaults, numVaultGroups, curUnbondedVaultGroup, _vcs.vaultMaxDeposits(), depositIndex);
     ...
     */
    function _getTotalUnbonded(address[] memory _vaults, uint256 _numVaultGroups, uint256 _vaultGroup)
        internal
        view
        returns (uint256)
    {
