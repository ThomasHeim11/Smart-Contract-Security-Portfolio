...
contract OperatorVault is Vault {
     ...
     */
    //@audit onlyVaultController be overwritten???
    ...
    function getPrincipalDeposits() public view override returns (uint256) {
        return
            super.getPrincipalDeposits() + IOperatorStaking(address(stakeController)).getRemovedPrincipal(address(this));
    }
    ...
    function _withdrawRewards() private {
        ...
        uint256 amountWithdrawn =
            IOperatorVCS(vaultController).withdrawOperatorRewards(rewardsReceiver, rewards - balance);
